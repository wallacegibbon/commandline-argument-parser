...
int
cmd_argument_parser_step (struct cmd_argument_parser *self)
{
...
void
cmd_argument_parser_prepare (struct cmd_argument_parser *self, int argc,
			     const char **argv)
{
  self->argc = argc;
  self->argv = argv;
...
void
cmd_argument_parser_cleanup (struct cmd_argument_parser *self)
{
  free (self->keys);
  free (self->values);
  free (self->others);
...
const char *
cmd_argument_parser_get (struct cmd_argument_parser *self, const char *key,
...
int
cmd_argument_parser_has (struct cmd_argument_parser *self, const char *key)
{
  int i;
  for (i = 0; i < self->option_count; i++)
    {
...
void
cmd_argument_parser_describe (struct cmd_argument_parser *self)
{
  int i;
  for (i = 0; i < self->option_count; i++)
